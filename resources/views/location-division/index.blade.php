        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-base text-black bg-gray-200 dark:bg-gray-700 text-center">
                    <tr>
                        <th class="px-6 py-3 font-semibold">Employee Name</th>
                        <th class="px-6 py-3 font-semibold">Company</th>
                        <th class="px-6 py-3 font-semibold">Location</th>
                        <th class="px-6 py-3 font-semibold">Work Type</th>
                        <th class="px-6 py-3 font-semibold">Work Detail</th>
                ...
                <tbody>
                    @forelse ($locationDivision as $data)
                        <tr class="odd:bg-white even:bg-gray-50 text-center hover:bg-gray-100">
                            <td class="px-6 py-4 text-gray-900 font-medium text-sm">{{ $data->employee->name }}</td>
                            <td class="px-6 py-4 text-sm">{{ $data->cooperation->company_name }}</td>
                            <td class="px-6 py-4 text-sm">{{ $data->location->location }}</td>
                            <td class="px-6 py-4 text-sm">{{ $data->work->work_type }}</td>
                            <td class="px-6 py-4 text-sm">{{ $data->work_detail }}</td>
                            <td class="px-6 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500 text-sm">Tidak ada data ditemukan.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
