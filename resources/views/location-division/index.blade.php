        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400 text-center">
                    <tr>
                        <th scope="col" class="px-6 py-3 font-bold">Employee Name</th>
                        <th scope="col" class="px-6 py-3 font-bold">Company</th>
                        <th scope="col" class="px-6 py-3 font-bold">Location</th>
                        <th scope="col" class="px-6 py-3 font-bold">Job Type</th>
                        <th scope="col" class="px-6 py-3 font-bold">Job Detail</th>
                ...
                <tbody>
                    @forelse ($locationDivision as $data)
                        <tr class="odd:bg-white even:bg-gray-50 border-b dark:border-gray-700 text-center">
                            ...
                            <td class="px-6 py-4 text-center">{{ $data->company }}</td>
                            <td class="px-6 py-4 text-center">{{ $data->location }}</td>
                            <td class="px-6 py-4 text-center">{{ $data->work_type }}</td>
                            <td class="px-6 py-4 text-center">{{ $data->work_detail }}</td>
                            <td class="px-6 py-4 text-center">
                                <span
                                    ...
                                    @elseif($data->status == 'on_leave') bg-yellow-500 text-black
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('location-division.edit', $data->id) }}"
                                        class="bg-yellow-500 p-2 rounded" title="Edit">
                                    ...
                                    <form action="{{ route('location-division.destroy', $data->id) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-600 p-2 rounded" title="Hapus">
