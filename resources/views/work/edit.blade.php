...
<div class="max-w-xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md">
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Edit Work</h2>
    ...
    <form action="{{ route('work.update', $work->id) }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="job_name" id="job_name"
                value="{{ old('job_name', $work->job_name) }}"
        ...
        <div class="mb-5">
            ...
            <input type="text" name="task_type" id="task_type" 
                  value="{{ old('task_type', $work->task_type) }}"
                   class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-base 
                          focus:ring-blue-500 focus:border-blue-500 
                          dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white 
                          dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <div class="mb-6">
<div class="mb-6">
    <label for="task_details" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Task Details</label>
    <textarea name="task_details" id="task_details" rows="6"
        class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm
        focus:ring-blue-500 focus:border-blue-500 
        dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white 
        dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ old('task_details', $work->task_details) }}</textarea>
</div>

        {{-- Gaji Per Orang  --}}
        <div class="mb-5">
            <label for="salary_per_person" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Salary per Person</label>
            <select id="salary_per_person" name="salary_per_person"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                     ...
                     <option value="500-900 Ribu" {{ old('salary_per_person', $work->salary_per_person) == '500-900 Ribu' ? 'selected' : '' }}>500-900 Ribu</option>
                     <option value="1-3 Juta" {{ old('salary_per_person', $work->salary_per_person) == '1-3 Juta' ? 'selected' : '' }}>1-3 Juta</option>
                     <option value="4-7 Juta" {{ old('salary_per_person', $work->salary_per_person) == '4-7 Juta' ? 'selected' : '' }}>4-7 Juta</option>
                     <option value="8-10 Juta" {{ old('salary_per_person', $work->salary_per_person) == '8-10 Juta' ? 'selected' : '' }}>8-10 Juta</option>
                 </select>
          </div>          
