...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <form action="{{ route('processing_wd.update', $processing_wd->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="task_name" name="task_name" value="{{ old('task_name', $processing_wd->task_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="employee_name" name="employee_name" value="{{ old('employee_name', $processing_wd->employee_name) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="inprogress" {{ $processing_wd->status == 'inprogress' ? 'selected' : '' }}>In Progress</option>
                    <option value="completed" {{ $processing_wd->status == 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
            </div>
            ...
            <div>
                ...
                @if($processing_wd->photo_before_path)
                    <div class="mt-2">
                        <img src="{{ $processing_wd->photo_before_url }}" alt="Before photo preview" class="w-full h-48 object-cover rounded-lg">
                    </div>
                @else
            ...
            <div>
                ...
                @if($processing_wd->photo_after_path)
                    <div class="mt-2">
                        <img src="{{ $processing_wd->photo_after_url }}" alt="After photo preview" class="w-full h-48 object-cover rounded-lg">
                    </div>
                @else
                    <div class="mt-2 border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                        <p class="text-gray-500">No photo available</p>
