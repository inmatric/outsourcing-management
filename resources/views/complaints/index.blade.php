...
@section('content')
    ...
    <div class="mb-6">
        ...
        <form action="{{ route('complaints.index') }}" method="GET" class="mb-6">
            <div class="flex items-center gap-3">
                ...
                @if (request()->has('location'))
                    <a href="{{ route('complaints.index') }}" class="text-sm text-gray-600 underline hover:text-gray-800">
                        Reset
                    </a>
                @endif
            </div>
        ...
        <a href="{{ route('complaints.create') }}"
            class="text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-base px-6 py-3 transition-all duration-300 ease-in-out transform hover:scale-105 shadow-lg">
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4">
                            @if ($complaint->proof_image)
                                ...
                                @else
                                    <img src="{{ asset('storage/' . $complaint->proof_image) }}" alt="Proof"
                        ...
                        <td class="px-6 py-4">
                            ...
                            @if ($complaint->resolution)
                                {{-- pastikan relasi `resolution` sudah di-define di model Complaint --}}
                                <span
                                    class="inline-block text-xs font-semibold px-3 py-1 rounded-full shadow-sm ring-1 {{ $statusColors[$complaint->resolution->status] ?? 'bg-gray-100 text-gray-800 ring-gray-300' }}">
                                    {{ ucfirst($complaint->resolution->status) }}
                                </span>
                            @else
                                <span class="text-gray-400 italic">Belum ditindak</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-center align-middle">
                            <div class="flex justify-center items-center space-x-3">
                                ...
                                <a href="{{ route('complaints.edit', $complaint->id) }}"
                                    class="inline-flex items-center px-3 py-1 text-sm font-medium text-white bg-yellow-500 hover:bg-yellow-600 rounded">
