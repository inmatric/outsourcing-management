        ...
        <form action="{{ route('employees.update', $employee->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-5">
                ...
                <input
                    ...
                    aria-describedby="file_input_help" id="file_input" type="file" name="card_id" />
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">SVG, PNG, JPG or GIF (MAX.
                    800x400px).</p>
                <div class="mt-2">
                    @if($employee->card_id)
                        <a href="{{ asset('storage/' . $employee->card_id) }}" target="_blank"
                            class="text-blue-600 hover:underline">
                            Lihat ID Kartu
                        </a>
                    @else
                        <span class="text-gray-500">Belum ada ID Kartu</span>
                    @endif
                </div>
                @error('card_id')
                    <span class="text-red-500 text-xs">{{ $message }}</span>
